...
function createPdfRoute (req, res) {
	...
	const items =[
		{description: 'أنا أواجه مشكلة في تنزيل pdf 1', quantity: 1, price: 20.10, total: 20.10},
	   {description: 'أنا أواجه مشكلة في تنزيل pdf 2', quantity: 4, price: 4.00, total: 16.00},
...
function createPDF (items, receiptNumber, errback) {
	...
	const tableBody = [];
	tableBody.push(['description', 'quantity', 'price', 'total']);
	let totalPrice = 0;
	items.forEach((item) => {
		// const itemPrice = parseFloat(item.price, 10);
		// totalPrice += itemPrice;
		const itemInfo = [item.description, item.quantity, item.quantity, item.total];
